...
const DocumentsPreviewPage: React.FC = () => {
  ...
  const maintainDocumentTexts = async () => {
    ...
    if (promises.length > 0) {
      ...
      try {
        ...
        toast.error("Не удалось прочитать содержимое документа");
      }
  ...
  const handleCreatePresentation = async () => {
    try {
      setShowLoading({
        message: "Генерируем структуру презентации...",
        show: true,
        duration: 40,
        progress: true,
      });
      ...
      toast.error("Ошибка", {
        description: error.message || "Не удалось создать презентацию.",
      });
      setShowLoading({
        message: "Не удалось создать презентацию.",
        show: true,
        duration: 10,
        progress: false,
      });
      ...
      <div className="h-full mr-4">
        <div className="overflow-y-auto custom_scrollbar h-full">
          <div className="h-full w-full max-w-full flex flex-col mb-5">
            <h1 className="text-2xl font-medium mb-5">Содержимое:</h1>
            ...
            <div className="flex flex-col gap-2 mt-6">
              {documentKeys.map((key: string) => (
                <div
                  ...
                  className={`${selectedDocument === key ? "border border-blue-500" : ""} flex p-2 rounded-sm gap-2 items-center cursor-pointer`}
                >
                  <img className="h-6 w-6 border border-gray-200" src={getIconFromFile(key)} alt="Иконка документа" />
        ...
        {!isOpen && (
          <div className="fixed left-4 top-1/2 -translate-y-1/2 z-50">
            <ToolTip content="Открыть панель">
              <Button onClick={() => setIsOpen(true)} className="bg-[#5146E5] text-white p-3 shadow-lg">
