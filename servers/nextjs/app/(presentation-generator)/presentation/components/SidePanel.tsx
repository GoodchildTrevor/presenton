...
const SidePanel = ({
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = presentationData?.slides.findIndex((item: any) => item.id === active.id);
      const newIndex = presentationData?.slides.findIndex((item: any) => item.id === over.id);
      const reorderedArray = arrayMove(presentationData?.slides, oldIndex, newIndex);
      const updatedArray = reorderedArray.map((slide: any, index: number) => ({ ...slide, index }));
      dispatch(setPresentationData({ ...presentationData, slides: updatedArray }));
    }
  };
      ...
      {!isOpen && (
        <div className="hidden xl:block fixed left-4 top-1/2 -translate-y-1/2 z-50">
          <ToolTip content="Открыть панель">
            <Button onClick={() => setIsOpen(true)} className="bg-white hover:bg-gray-50 shadow-lg">
          ...
          <div className="sticky top-0 z-40 px-6 py-4">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center justify-start gap-4">
                <ToolTip content="Просмотр миниатюр">
                  <Button
                    className={`${active === "grid" ? "bg-[#5141e5] hover:bg-[#4638c7]" : "bg-white hover:bg-white"}`}
                    onClick={() => { if (!isStreaming) setActive("grid"); }}
                  >
                    <LayoutList className={`${active === "grid" ? "text-white" : "text-black"}`} size={20} />
                  </Button>
                </ToolTip>
                <ToolTip content="Просмотр списком">
                  <Button
                    className={`${active === "list" ? "bg-[#5141e5] hover:bg-[#4638c7]" : "bg-white hover:bg-white"}`}
                    onClick={() => { if (!isStreaming) setActive("list"); }}
                  >
                    <ListTree className={`${active === "list" ? "text-white" : "text-black"}`} size={20} />
                  </Button>
                </ToolTip>
              </div>
              <X onClick={handleClose} className="text-[#6c7081] cursor-pointer hover:text-gray-600" size={20} />
            </div>
          </div>
              ...
              <div className="p-4 overflow-y-auto hide-scrollbar h-[calc(100%-100px)]">
                {isStreaming ? (
                  presentationData?.slides.map((slide: any, index: number) => (
                    <div key={`${index}-${slide.type}-${slide.id}`} className="p-3 cursor-pointer rounded-lg slide-box">
                      <span className="font-medium slide-title">Слайд {index + 1}</span>
                      <p className="text-sm slide-description">{slide.content.title}</p>
                    </div>
                  ))
                ) : (
                  <SortableContext
                    items={presentationData?.slides.map((slide: any) => slide.id!) || []}
            ...
            {active === "grid" && (
              <div className="p-4 overflow-y-auto hide-scrollbar h-[calc(100%-100px)] space-y-4">
                {isStreaming ? (
                  ...
                  <SortableContext
                    items={presentationData?.slides.map((slide: any) => slide.id || `${slide.index}`) || []}
                    strategy={verticalListSortingStrategy}
                  >
                    {presentationData?.slides.map((slide: any, index: number) => (
                      <SortableSlide
