...
const layoutName = "Показатели в цифрах"
const layoutDescription = "Swift: Наше влияние в цифрах с тремя вертикальными карточками показателей"

const MetricSchema = z
  .object({
    value: z.string().min(1).max(8).default("10K+"),
    line1: z.string().min(2).max(22).default("Всего"),
    line2: z.string().min(0).max(22).default("Пользователей"),
    description: z
      .string()
      .min(10)
      .max(140)
      .default("активных пользователей в различных отраслях"),
  })
  .default({
    value: "10K+",
    line1: "Всего",
    line2: "Пользователей",
    description: "активных пользователей в различных отраслях",
  })
...
const Schema = z
  .object({
    title: z
      ...
      .min(8)
      .max(60)
      .default("Наше влияние в цифрах"),
    leftTitle: z
      .string()
      .min(6)
      .max(40)
      .default("Доказанные результаты\nчерез данные"),
    leftBody: z
      .string()
      .min(30)
      .max(220)
    ...
    metrics: z
      ...
      .default([
        ...
        MetricSchema.parse({
          ...
          line1: "Рост",
          line2: "Выручки",
          description: "ежегодный прирост выручки компании",
        }),
        MetricSchema.parse({
          value: "95%",
          line1: "Уровень",
          line2: "Лояльности",
          description: "показатель удержания клиентов со средним рейтингом 4.8/5",
        }),
      ]),
  })
  .default({
    title: "Наше влияние в цифрах",
    leftTitle: "Доказанные результаты\nчерез данные",
    leftBody: "Далеко-далеко за словесными горами в стране гласных и согласных живут рыбные тексты. Вдали от всех живут они в буквенных домах.",
    ...
    <>
      ...
      >
        ...
        <div className="px-12 pt-3 grid grid-cols-[42%_58%] gap-8 items-start">
          {/* Левый контент */}
          <div>
            <h1 className="text-[48px] leading-[1.1] font-semibold max-w-[420px]" style={{ color: "var(--text-heading-color, #111827)" }}>
              {slideData?.title}
            </h1>
            <div className="mt-8 inline-flex items-center gap-3">
              <div className="w-5 h-5 rounded-full" style={{ backgroundColor: "var(--text-heading-color, #111827)" }}></div>
              <div>
                <div className="text-[20px] font-semibold whitespace-pre-line" style={{ color: "var(--text-heading-color, #111827)" }}>
                  {slideData?.leftTitle}
                </div>
              </div>
            </div>
            <p className="mt-5 text-[16px] leading-[1.8] max-w-[360px]" style={{ color: "var(--text-body-color, #6B7280)" }}>
              {slideData?.leftBody}
            </p>
          </div>

          {/* Правые карточки показателей */}
