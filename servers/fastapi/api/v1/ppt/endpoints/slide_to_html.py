...
async def generate_html_from_slide(
    ...
    """
    Generate HTML content from slide image and XML using OpenAI chat.completions API.
    ...
    """
    print("Generating HTML from slide image and XML...")
    try:
        ollama_base = (get_ollama_url_env() or "http://localhost:11434") + "/v1"
        is_ollama = get_llm_provider_env() and get_llm_provider_env().lower() == "ollama"
        ...
        print("Making chat.completions request for HTML generation...")

        if is_ollama and vision_model:
            # Ollama branch: chat.completions with image_url / text types
            ...
            # OpenAI branch: chat.completions with image_url / text types
...
async def generate_react_component_from_html(
    ...
    Args:
        ...
        media_type: MIME type of the optional image
    ...
    try:
        ...
        print("Making chat.completions request for React component generation...")
...
async def edit_html_with_images(
    ...
    try:
        ...
        print("Making chat.completions request for HTML editing...")
        ...
        # Build content_parts with chat.completions format: image_url / text
        content_parts = [
...
async def convert_slide_to_html(request: SlideToHtmlRequest):
    """
    Convert a slide image and its OXML data to HTML using OpenAI API.
...
async def convert_html_to_react(request: HtmlToReactRequest):
    """
    Convert HTML content to TSX React component using OpenAI API.
...
async def edit_html_with_images_endpoint(
    ...
    """
    Edit HTML content based on one or two uploaded images and a text prompt using OpenAI API.
